(function ($) {
    var apiURL = 'https://company.vietnambestplacestowork.com/api/export-company-award/{award}/{year}?x_auth_token=AwHBFADBohjsnZj6lK7yRM9lI1OH5EZx'
    ...
    function getDataURL() {
        let award = jQuery('#award').val();
        let year = jQuery('#select_year').val();
        let url = new URL(window.location.href);
            
        let industry = url.searchParams.get('industry');
        let dataURL = dataURL.replace('{year}', year);
        if (industry) {
            dataURL = apiURL.replace('{award}', industry)    
        }
        else {
            dataURL = apiURL.replace('{award}', award);
        }
        console.log(dataURL);
        return dataURL;
    }
